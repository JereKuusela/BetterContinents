...
public partial class BetterContinents
{
    ...
    public class WorldGeneratorPatch
    {
        ...
        private static void GetBiomeHeightPostfix(WorldGenerator __instance, float wx, float wy, ref float __result, World ___m_world)
        {
            ...
            float smoothHeight = __instance.GetBaseHeight(wx, wy, false) * 200f;
        ...
        [HarmonyPrefix, HarmonyPatch(nameof(WorldGenerator.GetBiome), typeof(float), typeof(float)), HarmonyAfter("expand_world_data")]
        ...
        while (!__instance.m_stop)
        {
            ...
            else
            {
                ...
                var buildTasks = __instance.m_toBuild
                    .Select(b => new
                    {
                        task = Task.Run(() => __instance.Build(b)),
                        buildData = b
                    })
