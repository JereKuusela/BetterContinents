...
internal class ImageMapLocation(string path) : ImageMapBase(path)
{
    ...
    public static ImageMapLocation? Load(ZPackage pkg, int version)
    {
        var path = pkg.ReadString();
        if (string.IsNullOrEmpty(path))
            return null;
        var map = new ImageMapLocation(path);
